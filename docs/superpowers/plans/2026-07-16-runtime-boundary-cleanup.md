...
- [ ] **Step 1: Move the four helper tests to `tests/runtime/agent-runtime.test.ts` and import the not-yet-created runtime module.**
- [ ] **Step 2: Run `npx.cmd vitest run tests/runtime/agent-runtime.test.ts` and verify it fails because the module is missing.**
- [ ] **Step 3: Create `agent-runtime.ts` with the existing helper implementations and required imports.**
- [ ] **Step 4: Run the runtime and CLI tests and verify they pass.**
...
- [ ] **Step 1: Make `chat.ts` import the runtime helpers and retain only terminal orchestration.**
- [ ] **Step 2: Change Electron imports from `../../cli/chat.js` to `../runtime/agent-runtime.js`.**
- [ ] **Step 3: Run CLI, chat IPC, typecheck, and build verification.**
...
- [ ] **Step 1: Delete the unused implementation and its dedicated test.**
- [ ] **Step 2: Add a current-state note to the Phase 1 learning document.**
- [ ] **Step 3: Verify no production or test code references `minimal-agent`.**
- [ ] **Step 4: Run `npm.cmd test`, `npm.cmd run typecheck`, and `npm.cmd run build`.**
